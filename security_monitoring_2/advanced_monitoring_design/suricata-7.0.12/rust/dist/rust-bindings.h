...
struct JA4 *SCJA4New(void);

void SCJA4SetTLSVersion(struct JA4 *j, uint16_t version);

void SCJA4AddCipher(struct JA4 *j, uint16_t cipher);

void SCJA4AddExtension(struct JA4 *j, uint16_t ext);

void SCJA4AddSigAlgo(struct JA4 *j, uint16_t sigalgo);

void SCJA4SetALPN(struct JA4 *j, const char *proto, uint16_t len);

void SCJA4GetHash(struct JA4 *j, uint8_t (*out)[36]);

void SCJA4Free(struct JA4 *j);
...
int rs_smb_state_get_event_info_by_id(int event_id,
                                      const char **event_name,
                                      enum AppLayerEventType *event_type);
...
bool SCPgsqlLogger(void *tx, uint32_t flags, struct JsonBuilder *js);

/**
 * C entry point for a probing parser.
 */
AppProto SCPgsqlProbingParserTS(const Flow *_flow,
                                uint8_t _direction,
                                const uint8_t *input,
                                uint32_t input_len,
                                uint8_t *_rdir);
 ...
 * C entry point for a probing parser.
 */
AppProto SCPgsqlProbingParserTC(const Flow *_flow,
                                uint8_t _direction,
                                const uint8_t *input,
                                uint32_t input_len,
                                uint8_t *_rdir);

void *SCPgsqlStateNew(void *_orig_state, AppProto _orig_proto);

void SCPgsqlStateFree(void *state);

void SCPgsqlStateTxFree(void *state, uint64_t tx_id);

struct AppLayerResult SCPgsqlParseRequest(const Flow *flow,
                                          void *state,
                                          void *pstate,
                                          struct StreamSlice stream_slice,
                                          const void *_data);

struct AppLayerResult SCPgsqlParseResponse(const Flow *flow,
                                           void *state,
                                           void *pstate,
                                           struct StreamSlice stream_slice,
                                           const void *_data);

void *SCPgsqlStateGetTx(void *state, uint64_t tx_id);

uint64_t SCPgsqlStateGetTxCount(void *state);

int SCPgsqlTxGetALStateProgress(void *tx, uint8_t direction);
...
uint8_t rs_quic_tx_get_ja4(const struct QuicTransaction *tx,
                           const uint8_t **buffer,
                           uint32_t *buffer_len);
...
bool rs_quic_to_json(void *tx, bool log_ja4, struct JsonBuilder *js);
