...
def get_age_group(age):
    """
    Determines the age group a person belongs to based on their age.

    Parameters:
    age (int): The age of the person.

    Returns:
    str or None: Returns a string representing the age group ("18-25", "26-30", "31-39") 
                 if the age falls within these ranges. Returns None if the age is outside these ranges.
    """
    if 18 <= age <= 25:
        return "18-25"
    elif 26 <= age <= 30:
...
def get_time_bucket(time_str):
    """
    Categorizes the time of day based on the given time string.

    Parameters:
    time_str (str): Time in "HH:MM" format.

    Returns:
    str: Returns one of the categories "morning", "afternoon", or "evening" 
         based on the hour extracted from the time string.
    """
