...
def select_photo():
    ...
    if path:
        ...
        photo_preview_label.image = dog_photo_imgtk # Keep reference

        # If currently on the first question (Dog's Name), refresh its view
        # to correctly pack the photo_preview_label within its designated frame.
        # Assuming the first question (index 0) is "Dog's Name".
        if current_question_index == 0 and questions[current_question_index][0] == "Dog's Name":
            show_question()
...
def get_recommendations():
    ...
    water = answers.get("Likes Swimming?")
...
def show_summary():
    ...
    progress_var.set(current_question_index) # Should be len(questions) when summary is shown after all questions
    if current_question_index >= len(questions): # ensure progress bar is full at summary
        progress_var.set(len(questions))

    message_frame.pack_forget() # Usually hidden before summary
    welcome_frame.pack_forget()
    ...
    # Repack question_frame for summary content
...
def restart():
    ...
    progress_frame.pack_forget()
    progress_var.set(0)
    message_frame.pack_forget() # Hide message frame initially
    for widget in message_frame.winfo_children(): # Clear any existing messages
        widget.destroy()
    welcome_frame.pack(padx=10, pady=30, fill="both", expand=True)
...
def previous_question():
    ...
    if current_question_index > 0:
        current_question_index -= 1
        progress_var.set(current_question_index) # Update progress bar
        show_question() # Display the now current (previous) question

def show_question():
    for widget in question_frame.winfo_children():
        widget.destroy()
    photo_preview_label.pack_forget() # Ensure it's hidden from any previous packing

    question_key, options = questions[current_question_index]
    tk.Label(question_frame, text=question_key, font=("Helvetica", 13), bg="#f7f7f7").pack(pady=10)

    # --- Main Content Area (Entry or Option Buttons) ---
    content_area = tk.Frame(question_frame, bg="#f7f7f7")
    content_area.pack(pady=10, fill="both", expand=True) # Fill and expand for better layout

    # This local_name_var is specific to the "Dog's Name" question scope
    local_name_var = None
    name_entry_widget = None # To store reference to name_entry for focus

    if question_key == "Dog's Name":
        # Dog's Name Entry
        local_name_var = tk.StringVar(value=answers.get(question_key, ""))
        name_entry_widget = tk.Entry(content_area, font=("Helvetica", 12), width=30, textvariable=local_name_var)
        name_entry_widget.pack(pady=5)
        name_entry_widget.focus()

        # Photo Preview (shown if a photo has been uploaded)
        if dog_photo_imgtk:
            photo_preview_label.config(image=dog_photo_imgtk) # Ensure image is up-to-date
            photo_preview_label.pack(in_=content_area, pady=5) # Pack within content_area
        else:
            photo_preview_label.pack_forget()

        # Upload Photo Button
        tk.Button(content_area, text="Upload Dog Photo (Optional)", command=select_photo, width=25, bg="#6fa8dc").pack(pady=10)
    else:
        # Option Buttons for other questions
        option_buttons_frame = tk.Frame(content_area, bg="#f7f7f7")
        option_buttons_frame.pack() # Pack this frame to center its content (buttons)

        def on_option_click(opt): # Helper defined here to capture opt correctly
            next_question(opt)

        for opt in options:
            b = tk.Button(option_buttons_frame, text=opt, width=20, command=lambda o=opt: on_option_click(o), bg="#8dd694")
            b.pack(pady=3)

    # --- Unified Navigation Frame (at the bottom of question_frame) ---
    nav_frame = tk.Frame(question_frame, bg="#f7f7f7")
    nav_frame.pack(side="bottom", fill="x", pady=(5,10), padx=10) # Add some padding

    # Previous Button (always present, but state changes)
    prev_button = tk.Button(nav_frame, text="Previous", command=previous_question, width=12, bg="#aec6cf") # Light blueish color
    ...
    # Next Button (only for "Dog's Name" question)
    if question_key == "Dog's Name":
        next_button_name = tk.Button(nav_frame, text="Next", width=12, bg="#8dd694") # Green color
        next_button_name.pack(side="right", padx=(5, 10))

        def _update_next_button_state_for_name(*args):
            if local_name_var: # Check if local_name_var is initialized
                name_val = local_name_var.get().strip()
                if name_val:
                    next_button_name.config(state="normal")
                    show_message("") # Clear any previous error message
                else:
                    next_button_name.config(state="disabled")

        def _submit_dog_name_and_proceed():
            if local_name_var: # Check if local_name_var is initialized
                name_val = local_name_var.get().strip()
                if not name_val:
                    show_message("Please enter your dog's name.", error=True)
                    if name_entry_widget and name_entry_widget.winfo_exists():
                        name_entry_widget.focus()
                    return
                next_question(name_val)

        if local_name_var: # Ensure local_name_var was set (i.e., it's the Dog's Name question)
             local_name_var.trace_add("write", _update_next_button_state_for_name)
             next_button_name.config(command=_submit_dog_name_and_proceed)
             _update_next_button_state_for_name() # Call once to set initial state
        else: # Fallback if local_name_var somehow isn't defined, disable button
            next_button_name.config(state="disabled")

    show_message("") # Clear any stale messages from previous question views
...
welcome_msg.pack(pady=60) # Removed expand=True, fill="both" to keep it centered
...
def start_quiz():
    welcome_frame.pack_forget()
    message_frame.pack(fill="x", pady=2) # Make message frame visible
    question_frame.pack(pady=10, fill="both", expand=True) # Make question frame take space
    progress_frame.pack(fill="x", pady=5, padx=10) # Progress bar frame
    progress_var.set(0)
    show_question()
...
# message_frame is packed by start_quiz

# Question frame
question_frame = tk.Frame(root, bg="#f7f7f7")
# question_frame is packed by start_quiz
...
progress_bar.pack(fill="x", expand=True) # Make progress bar fill its frame
# progress_frame is packed by start_quiz

# Result frame
result_frame = tk.Frame(root, bg="#f7f7f7")
# result_frame is packed by show_recommendations_only

# Photo preview label (hidden initially, managed by show_question)
photo_preview_label = tk.Label(root, bg="#f7f7f7") # Global label for photo preview

# Dynamic wraplength adjustment for welcome message on resize
def adjust_wraplength(event):
    # Adjust for welcome_msg if it's visible
    if welcome_msg.winfo_ismapped():
        new_wraplength = welcome_msg.master.winfo_width() - 40 # Adjust based on welcome_frame width
